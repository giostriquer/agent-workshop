...
Three failure families recur when a session builds a report page without the distilled conventions:
...
A rendering contract plus an editing discipline. The page architecture is fixed (single self-contained file, sticky TOC with keyboard nav, tables over prose walls, verified-links-only with inline result annotations, evidence appendix, print stylesheet). The design system — dark blue-gray canvas, bright sans-serif body, subtle card panels, one sparing accent family — is framed as **defaults**, adaptable when a document needs a different mood. The process rules are **rigid**: one-pass generation, targeted-edit vs clean-rewrite (a design-direction change always rewrites), one-knob-at-a-time feedback handling, a collision-safe renumbering procedure, and a pre-finish checklist (parse-check, TOC targets, nav-order array, no dropped content).

The skill also embeds compact reference markup for the two structures sessions kept reinventing — the card (label → headline → metadata → body) and the vertical stepper — so future sessions copy rather than improvise.
...
- **Unverified links.** A polished page full of links nobody fetched reads as authoritative and isn't. Verify or omit.
...
- The design system is the adaptable half: a light theme, a different accent family, or a denser layout are legitimate per-document choices. Keep the readability floors (bright text, readable chips, quiet structure) even when the mood changes.
- The process rules are the portable half — they apply unchanged to any theme, including light ones.
- The reference markup is a starting point, not a component library; adapt class names and tokens to taste, keep the shapes.
