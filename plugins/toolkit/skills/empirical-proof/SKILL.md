...
description: Use when a piece of work is finished and it touched a surface that can be proven by driving the running software — an MCP tool, a REST API endpoint, any behavior a real client can drive — and "done" should mean empirically verified rather than claimed. Gates on the app genuinely running before anything counts, fans scenario testing out to subagents under a raw-evidence contract, probes beyond the happy path, and corroborates firsthand before reporting a verdict-first verified / broken / blocked result. If the environment cannot run the app the honest outcome is blocked-with-cause — it never fixes local setup, and it reports the bugs it finds without fixing them. NOT for a release- or branch-wide pass (that is qa-sweep) and not for verifying a premise or ticket (that is claim-check) — this is the post-implementation, single-change runtime proof.
---
# Empirical Proof
